...
import EditChemistProfileModal from '../EditChemistProfileModal/EditChemistProfileModal';

const Chemist_Profile = (props) => {
  console.log(props, 'propsChemist_Profile')
  ...
  const getChemistData= useSelector((state)=> state?.ChemistProfileReducer?.ChemistProfileRegister?.data)
  console.log("getChemistDatagetChemistData",getChemistData)

   dispatch(GetChemistProfileAction(ids)).then(async (response) => {
    console.log("GetChemistProfileAction", response)
        ...
        <View style={{ flex: 0.3 }}>
          <View style={{ flex: 2, flexDirection: 'row', justifyContent: 'space-between', width: '90%' }}>
            <View style={{ width: '60%', justifyContent: 'center', alignItems: 'center' }}>
              <Text style={{ fontFamily: fontFamily.semiBold, fontSize: textScale(20), color: colors.blackColor, alignSelf: 'center' }}>{getChemistData.name_of_firm}</Text>
            </View>
            <EditChemistProfileModal props={props}/>
          </View>
          ...
          <View style={{ flex: 3, padding: moderateScale(20), flexDirection: 'row', justifyContent: 'space-between' }}>
            ...
            <View style={{ justifyContent: 'center' }}>
              ...
              <View style={{ flexDirection: "row", top: moderateScale(3) }}>
                ...
                <Text style={{ fontFamily: fontFamily.regular, fontSize: textScale(12), color: colors.blackColor, left: moderateScale(3) }}>{getChemistData.city}</Text>
              </View>

              <View style={{ top: moderateScale(10) }}>
                <ButtonComp text={getChemistData.mobile}
...
export default Chemist_Profile
