...
import EditDiagnosticProfile from '../EditDiagnosticProfile/EditDiagnosticProfile';
import { GetDiaganosticProfileAction } from '../../redux/Action/Diagnostic_CenterAction'

const Diagnostic_Profile = (props) => {
  ...
  const diagnosticData = useSelector((state) => state?.Diaganostic_ProfileReducer?.DiaganosticProfileRegister?.data)
  console.log("diagnosticData", diagnosticData)

  dispatch(GetDiaganosticProfileAction(ids)).then(async (response) => {
        ...
        <View style={{ flex: 0.3 }}>
          <View style={{ flex: 2, flexDirection: 'row', justifyContent: 'space-between', width: '90%' }}>
            <View style={{ width: '60%', justifyContent: 'center', alignItems: 'center' }}>
              <Text style={{ fontFamily: fontFamily.semiBold, fontSize: textScale(20), color: colors.blackColor, alignSelf: 'center' }}>{diagnosticData.name}</Text>
            </View>
            <EditDiagnosticProfile props={props} />
          </View>
          ...
          <View style={{ flex: 3, padding: moderateScale(20), flexDirection: 'row', justifyContent: 'space-between' }}>
            ...
            <View style={{ justifyContent: 'center' }}>
              ...
              <View style={{ flexDirection: "row", top: moderateScale(3) }}>
                ...
                <Text style={{ fontFamily: fontFamily.regular, fontSize: textScale(12), color: colors.blackColor, left: moderateScale(3) }}>{diagnosticData.address}</Text>
              </View>

              <View style={{ top: moderateScale(10) }}>
                <ButtonComp text=
                {diagnosticData.mobile}
...
export default Diagnostic_Profile
