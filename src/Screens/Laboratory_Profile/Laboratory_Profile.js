...
import { GetLaboratoryProfileAction } from '../../redux/Action/Laboratory_ProfileAction'
import EditLaboratoryProfileModal from '../EditLaboratoryProfileModal/EditLaboratoryProfileModal';
import navigationStrings from '../../Navigations/navigationStrings'
import { useNavigation } from '@react-navigation/native';

const Laboratory_Profile = (props) => {
  console.log(props, 'propspropsProLib')
  ...
  const getData = useSelector((state) => state?.Laboratory_ProfileReducer?.LaboratoryProfileRegister.laboratory)
  console.log("getDatagetDa", getData)

  dispatch(GetLaboratoryProfileAction(ids)).then(async (response) => {
  ...
  return (
    <WrapperContainer>
      <View style={{ flex: 1 }}>
        ...
        >
          <Image source={imagePath.icCheckBlack} style={{left:moderateScale(10)}}/>
        </TouchableOpacity>


        <View style={{ flex: 0.3 }}>
          <View style={{ flex: 2, flexDirection: 'row', justifyContent: 'space-between', width: '100%' }}>
            <View style={{ width: '60%' ,justifyContent:'center',left :moderateScale(10)}}>
              <Text style={{ fontFamily: fontFamily.semiBold, fontSize: textScale(20), color: colors.blackColor,alignSelf:'center' }}>{getData.name}</Text>
            </View>
            <EditLaboratoryProfileModal props={props}/>
            {/* </View> */}
          </View>
          ...
          <View style={{ flex: 3, padding: moderateScale(20), flexDirection: 'row', justifyContent: 'space-between' }}>
            ...
            <View style={{ justifyContent: 'center' }}>
              ...
              <View style={{ flexDirection: "row", top: moderateScale(3) }}>
                ...
                <Text style={{ fontFamily: fontFamily.regular, fontSize: textScale(12), color: colors.blackColor, left: moderateScale(3) }}>{getData.address}</Text>
              </View>

              <View style={{ top: moderateScale(10) }}>
                <ButtonComp text={getData.mobile}
...
export default Laboratory_Profile
