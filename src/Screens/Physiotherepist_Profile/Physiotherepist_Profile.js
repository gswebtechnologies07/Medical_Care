...
import EditLaboratoryProfileModal from '../EditLaboratoryProfileModal/EditLaboratoryProfileModal';
import navigationStrings from '../../Navigations/navigationStrings'
import { useNavigation } from '@react-navigation/native';
import { GetPhysiotherapistProfileAction } from '../../redux/Action/PhysiotherapistProfileAction'
import PhysiotherapistProfileModal from '../../Components/PhysiotherapistProfileModal';
import EditPhysiotherepist_Profile from '../EditPhysiotherepist_Profile/EditPhysiotherepist_Profile';

const Physiotherepist_Profile = (props) => {
  console.log(props, 'propsPhysiotherepist_Profile')

  const ids = props?.route?.params?.id
  const dispatch = useDispatch();
  const navigation = useNavigation();

  const physiotherapy = useSelector((state) => state?.Physiotherapist_ProfileReducer?.PhysiotherapistProfileRegister?.physiotherepist)
  console.log("physiotherapyphysiotherapy", physiotherapy)

  dispatch(GetPhysiotherapistProfileAction(ids)).then(async (response) => {
    // console.log("GetLaboratoryProfileActions", response)
  })
        ...
        <View style={{ flex: 0.3 }}>
          <View style={{ flex: 2, flexDirection: 'row', justifyContent: 'space-between', width: '90%' }}>
            <View style={{ width: '60%', justifyContent: 'center', alignItems: 'center' }}>
              <Text style={{ fontFamily: fontFamily.semiBold, fontSize: textScale(20), color: colors.blackColor, alignSelf: 'center' }}>{physiotherapy.name}</Text>
            </View>
            <EditPhysiotherepist_Profile props={props} />
          </View>
          ...
          <View style={{ flex: 3, padding: moderateScale(20), flexDirection: 'row', justifyContent: 'space-between' }}>
            ...
            <View style={{ justifyContent: 'center' }}>
              ...
              <View style={{ flexDirection: "row", top: moderateScale(3) }}>
                ...
                <Text style={{ fontFamily: fontFamily.regular, fontSize: textScale(12), color: colors.blackColor, left: moderateScale(3) }}>{physiotherapy.address}</Text>
              </View>

              <View style={{ top: moderateScale(10) }}>
                <ButtonComp text={physiotherapy.mobile}
...
export default Physiotherepist_Profile
